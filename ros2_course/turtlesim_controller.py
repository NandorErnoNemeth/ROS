...
class MinimalParam(rclpy.node.Node):
    def __init__(self):
        ...
        self.declare_parameter('angle_speed', '1')
        self.timer = self.create_timer(1, self.timer_callback)

    def get_speed(self) -> float:
        return float(self.get_parameter('speed').get_parameter_value().string_value)
    
    def get_angle_speed(self) -> float:
        return float(self.get_parameter('angle_speed').get_parameter_value().string_value)
...
class TurtlesimController(Node):
    ...
    def __init__(self):
        ...
        #self.params = MinimalParam()

        self.declare_parameter('speed', 1.0)
        self.declare_parameter('angle_speed', 1.0)
    ...
    def draw_poly_param(self, N, a):
        angle = 360 / N
        
        for i in range(N):
            omega = (self.get_parameter('speed').get_parameter_value().double_value)
            speed = (self.get_parameter('angle_speed').get_parameter_value().double_value)
